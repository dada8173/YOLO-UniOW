"""
Generate text embeddings for GroceryOWOD tasks using the UniOW text encoder.
Allows regenerating specific task embeddings after class list changes (e.g.,
removing a class from known categories so it stays unknown).
"""
import argparse
import os
...
def extract_grocery_feats(dataset_name: str = "GroceryOWOD", tasks=None) -> bool:
    """Generate text embeddings for the specified GroceryOWOD tasks."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Extract GroceryOWOD text embeddings")
    parser.add_argument("--dataset", default="GroceryOWOD", help="Dataset name (default: GroceryOWOD)")
    parser.add_argument(
        "--tasks",
        default="",
...
def main():
    ...
    success = extract_grocery_feats(dataset_name=args.dataset, tasks=tasks)
